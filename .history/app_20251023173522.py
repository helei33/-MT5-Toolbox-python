...
class TradeCopierApp(ThemedTk):
            ...
            for i, mvars in enumerate(self.master_vars_list):
                ...
                for key in settings_to_save['master']:
                    if key in mvars and hasattr(mvars[key], 'get'):
                        self.app_config.set(master_id, key, str(mvars[key].get()))
            
            for i, svars in enumerate(self.slave_vars_list):
                slave_id = f'slave{i+1}'
                for key in [k for k in settings_to_save['slave'] + ['strategy'] if k in svars and hasattr(svars[key], 'get')]:
                    if key == 'volume_mode':
    ...
    def toggle_login(self, account_id):
        ...
        if account_id in self.logged_in_accounts: # 注销逻辑
            ...
            # 登录前，先保存当前UI上的所有配置到内存
            self.save_config() 
            # 检查此账户的密码是否被修改过，如果是，则移除旧的“已验证”标记
            if self.app_config.get(account_id, 'password') != decrypt_password(self.last_known_good_config.get(account_id, 'password', fallback='')):
                self.verified_passwords.discard(account_id)
                log_queue.put(f"检测到账户 {account_id} 密码已更改，将在本次登录时重新验证。")
    ...
    def configure_strategy_for_account(self, account_id):
        ...
        if not strategy_info or not strategy_info.get('params_config'):
            ...
            return
        StrategyConfigWindow(self, self.app_config, account_id, strategy_name, strategy_info['params_config'])
