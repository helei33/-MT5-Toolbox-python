...
class DualMaCrossoverStrategy(BaseStrategy):
    ...
    def check_and_trade(self):
        ...
        if prev_fast_ma < prev_slow_ma and last_fast_ma > last_slow_ma:
            self.log("检测到金叉信号 (买入)。")
            should_open_new_buy = True
            if my_positions:
                # 如果有反向持仓，先平仓
                for pos in my_positions:
                    if pos.type == self.mt5.ORDER_TYPE_SELL:
                        self.log(f"发现反向持仓 (Sell Ticket: {pos.ticket})，正在平仓...")
                        self.close_position(pos.ticket, pos.volume, pos.symbol, pos.type, "Close for Buy Signal")
                    elif pos.type == self.mt5.ORDER_TYPE_BUY:
                        should_open_new_buy = False # 已有同向持仓，不再开仓
            
            if should_open_new_buy:
                self.log("无持仓，准备开立多单...")
        ...
        # 死叉信号: 上一根K线快线在慢线上方，当前K线快线在慢线下方
            ...
            self.log("检测到死叉信号 (卖出)。")
            should_open_new_sell = True
            if my_positions:
                # 如果有反向持仓，先平仓
                for pos in my_positions:
                    if pos.type == self.mt5.ORDER_TYPE_BUY:
                        self.log(f"发现反向持仓 (Buy Ticket: {pos.ticket})，正在平仓...")
                        self.close_position(pos.ticket, pos.volume, pos.symbol, pos.type, "Close for Sell Signal")
                    elif pos.type == self.mt5.ORDER_TYPE_SELL:
                        should_open_new_sell = False # 已有同向持仓，不再开仓

            if should_open_new_sell:
                self.log("无持仓，准备开立空单...")
